...
export class BookParcelComponent {
    ...
    getPatternError(field: string): string {
        ...
        if (field === 'phoneNumber' || field === 'alternatePhoneNumber')
            return 'Invalid phone number (Only 10 digit numbers)';
    ...
    onSubmit() {
        ...
        const requestPayload: BookingParcelRequest = {
            ...
            parcel: {
                ...
                description:
                    bookingData.parcelRequest.parcelContentsDescription!,
                expectedPickupTime: bookingData.parcelRequest.parcelPickupTime!,
                expectedDropofTime:
                    bookingData.parcelRequest.parcelDropoffTime!,
            ...
            receiverDetails: {
                ...
                alternateNumber:
                    bookingData.receiverDetails.alternatePhoneNumber ||
                    undefined,
        ...
        this.bookingService.createBooking(requestPayload).subscribe({
            next: (response) => {
                ...
                // Added defensive check and fallback for bookingId
                if (responseData) {
                    const id =
                        responseData.bookingId ||
                        responseData.trakingId ||
                        responseData.id;
                    if (id) {
                        this.bookingId.set(id);
                    } else {
                        console.error(
                            'Booking ID missing in response:',
                            responseData
                        );
                    }
                }
    ...
    goToPayment() {
        ...
        if (!bId) {
            alert(
                'Booking ID not found. Please try again or check My Bookings.'
            ); // Simple feedback
            return;
        }
